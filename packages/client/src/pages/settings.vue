...
const { scale, interactionCloseOnOutsideClick, showPanel, minimizePanelInteractive, expandSidebar, scrollableSidebar } =
...
function toggleTab(name: string, v: boolean) {
  if (v) hiddenTabs.value = hiddenTabs.value.filter((i) => i !== name);
  else hiddenTabs.value.push(name);
}

function toggleTabCategory(name: string, v: boolean) {
  if (v) hiddenTabCategories.value = hiddenTabCategories.value.filter((i) => i !== name);
  else hiddenTabCategories.value.push(name);
}

function togglePinTab(name: string) {
  if (pinnedTabs.value.includes(name)) pinnedTabs.value = pinnedTabs.value.filter((i) => i !== name);
  else pinnedTabs.value.push(name);
}
...
<template>
  <div h-full w-full of-auto px8 py6>
    <IconTitle class="mb-5 text-xl op75" icon="i-carbon-settings-adjust" text="DevTools Settings" />
    <div grid="~ md:cols-[repeat(auto-fit,minmax(16rem,1fr))] gap-x-10 gap-y-3" max-w-300>
      <div flex="~ col gap-2">
        ...
        <template v-for="[{ name, hidden }, tabs] of categories" :key="name">
          <VueCard v-if="tabs.length" p3 flex="~ col gap-1" :class="hidden ? 'op50 grayscale' : ''">
            <VueSwitch
            ...
            <template v-for="tab of tabs" :key="tab.name">
              ...
              >
                <div flex="~ gap-2" flex-auto items-center justify-start pr-4 text-sm>
                  ...
                  <button
                    class="flex items-center hover:(bg-active op100) px1 py1 text-sm op65"
                    @click.stop="togglePinTab(tab.name)"
                  >
                    <div
                      :class="pinnedTabs.includes(tab.name) ? ' i-carbon-pin-filled rotate--45' : ' i-carbon-pin op45'"
                    />
                  </button>
                </div>
              </VueSwitch>
      ...
      <div flex="~ col gap-2">
        ...
        <VueCard p4 flex="~ col gap-2">
          ...
          <div>
            <VueSelect
              v-model="scale"
              :options="scaleOptions.map(([label, value]) => ({ label, value }))"
              :button-props="{ outlined: true }"
            />
          </div>
          <div mx--2 my1 h-1px border="b base" op75 />
        ...
        <template v-if="enableFeatureSettings">
          ...
          <VueCard p4 flex="~ col gap-2">
            ...
            <div>
              <VueSelect
                v-model="minimizePanelInteractive"
                :button-props="{ outlined: true }"
                :options="minimizePanelInteractiveOptions"
                :placeholder="minimizePanelInteractiveLabel"
              />
            </div>
          </VueCard>
        </template>

        <h3 mt2 text-lg>Debug</h3>
        <div flex="~ gap-2">
