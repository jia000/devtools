...
import { toSubmit } from '@tmagic/devtools-kit';
...
const inputType = computed(() => {
  if (props.customType === 'date') return 'datetime-local';
  return '';
});
...
watchEffect(() => {
  if (escape.value) emit('cancel');
  else if (enter.value) emit('submit');
});
...
<template>
  <span class="flex-inline items-center gap4px">
    <VueInput
      v-model="value"
      :type="inputType"
      :variant="isWarning ? 'warning' : 'normal'"
      class="h25px px4px"
      :class="customType === 'date' ? 'w240px' : 'w120px'"
