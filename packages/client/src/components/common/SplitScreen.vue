...
const mainViewName = computed(() => getRouteTabName());
...
watch(
  ...
  (tab) => {
    ...
    if (typeof component === 'function') PageComponent.value = defineAsyncComponent(component as any);
    else PageComponent.value = component;
  },
  { immediate: true },
);
...
<template>
  <div h-full h-screen of-hidden>
    ...
    <div v-else class="h-full w-full $ui-fcc">
      <div>
        <span text-lg op50> Select a tab to start </span>
        <VueCard px4 py2 bg-base>
          <TabsGrid :categories="enabledTabs" target="side" :disabled-items="[mainViewName]" />
        </VueCard>
        <VueButton type="warning" outlined mt2 @click="close"> Close Split Screen </VueButton>
      </div>
    </div>
  </div>
</template>
