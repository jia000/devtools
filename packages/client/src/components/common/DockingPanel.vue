...
import { refreshCurrentPageData, rpc, useDevToolsState } from '@tmagic/devtools-core';
...
const activeAppRecordName = computed(
  () => appRecords.value.find((app) => app.value === activeAppRecordId.value)?.label ?? '',
);
  ...
  <div>
    ...
    <div px3 py2 flex="~ gap2">
      <template v-if="appRecords.length > 1">
        <VueSelect
          v-model="activeAppRecordId"
          :options="appRecords"
          :placeholder="activeAppRecordName || 'Toggle App'"
          :button-props="{ outlined: true, type: 'primary' }"
          @update:model-value="toggleApp"
        />
      </template>
      <VueButton outlined type="primary" @click="refreshPage"> Refresh Page </VueButton>
    </div>
  </div>
</template>
