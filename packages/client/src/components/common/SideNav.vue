...
const displayedTabs = computed(() =>
  sidebarScrollable.value || sidebarExpanded.value ? enabledTabs.value : categorizedInlineTabs.value,
);

onClickOutside(
  panel,
  (e) => {
    if (buttonDocking.value && e.composedPath().includes(buttonDocking.value)) return;
    if (buttonMoreTabs.value && e.composedPath().includes(buttonMoreTabs.value)) return;
...
<template>
  <div ref="containerRef" border="r base" flex="~ col items-start" class="$ui-z-max-override" h-full of-hidden bg-base>
    <div sticky top-0 z-1 w-full p1 bg-base border="b base">
      <VueDropdown
        placement="left-start"
        :distance="dropdownDistance"
        :skidding="5"
        trigger="click"
        :shown="showDocking"
        class="w-full"
      >
        <button
          ...
          :class="[sidebarExpanded ? 'rounded pl2.5' : 'rounded-xl']"
        >
          <div h-6 w-6>
            <img
          ...
          <template v-if="sidebarExpanded">
            <span text-lg font-600 text-base> DevTools </span>
            <div flex-auto />
    ...
    >
      ...
      >
        <button
          ...
          exact-active-class="!text-primary bg-active"
        >
          <TabIcon text-xl icon="i-carbon-overflow-menu-vertical" title="More tabs" :show-title="false" />
          <div
            absolute
            bottom-0
            right-0
